...
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts'; // Import Recharts components
    ...
    if (active && payload && payload.length) {
        ...
        return (
            <Box className=" p-3 rounded-lg custom-tooltip outline outline-2 outline-zinc-100 bg-zinc-500/55 shadow-glowLight dark:shadow-glowDark">
                ...
                <Text>Price : <span className='text-green-400'> ${price.toFixed(2)}</span></Text>
                <Text>Volume (24hr): <span className='text-white'>{volume.toFixed(2)}</span></Text>
    ...
    return (
        <Box horizontal className="text-responsive-content p-4">
            <Box className="gap-4 w-[50%]">
                ...
                />
                <Text className="text-info">Selected Range: Last {selectedRange} Days</Text>

                {/* Display Data for Selected Range */}
                {selectedRangeData.length > 0 && (
                    <>
                        <Text className="text-info">Price:</Text>
                        <Text className="text-yellow-400">${selectedRangeData[0]?.price.toFixed(6)}</Text>
                        <Text className="text-info">Volume:</Text>
                        <Text className="text-yellow-400">{selectedRangeData[0]?.volume_24h}</Text>
                        <Text className="text-info">Selected Date:</Text>
                        <Text className="text-yellow-400">{oldestDate && formatDate(oldestDate)}</Text>
