...
const Details = ({ name, symbol, rank, logo, price, change, updatedAt }) => {
  const [loading, setLoading] = useState(true); // Track loading state
  const [error, setError] = useState(null); // Error state

  // Format change color based on value
  ...
  useEffect(() => {
    if (!name || !symbol || !rank || !price || !change || !updatedAt) {
      setError("Missing required data for displaying details.");
      setLoading(false);
      return;
    }

    setLoading(false); // Set loading to false when data is available
  }, [name, symbol, rank, price, change, updatedAt]);
  ...
  return (
    <Box className="gap-2 text-responsive-content">
      ...
      <Text className="font-semibold text-success">
        ${parseFloat(price[0].price).toFixed(3)}
      </Text>
      <Text className="text-info font-bold">Changes [24Hrs]</Text>
      <Text className={`font-semibold ${changeColor}`}>
        {" "}
